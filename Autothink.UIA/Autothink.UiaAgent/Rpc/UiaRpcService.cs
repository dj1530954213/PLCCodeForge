...
internal sealed class UiaRpcService
{
    ...
    /// 按锚点坐标系偏移点击（左键）。
    /// </summary>
    public RpcResult ClickAt(ClickAtRequest? request)
    {
        return PerformAnchorOffsetClick(
            request,
            request?.OffsetX ?? 0,
            request?.OffsetY ?? 0,
            stepId: "ClickAt",
    ...
    /// 按锚点坐标系偏移右键点击。
    /// </summary>
    public RpcResult RightClickAt(RightClickAtRequest? request)
    {
        return PerformAnchorOffsetClick(
            request,
            request?.OffsetX ?? 0,
    ...
    /// 按锚点矩形比例点击（左键）。
    /// </summary>
    public RpcResult ClickRel(ClickRelRequest? request)
    {
        var result = new RpcResult();
        StepLogEntry validateStep = StartStep(result.StepLog, stepId: "ValidateRequest", action: "Validate ClickRel request");
        ...
        if (request.AnchorSelector is null || request.AnchorSelector.Path.Count == 0)
        {
            return Fail(result, validateStep, RpcErrorKinds.InvalidArgument, "AnchorSelector must be provided");
        }
        ...
        if (request.XRatio < 0 || request.XRatio > 1 || request.YRatio < 0 || request.YRatio > 1)
        {
            return Fail(result, validateStep, RpcErrorKinds.InvalidArgument, "XRatio/YRatio must be within [0,1]");
        }
        ...
        if (!TryResolveAnchor(result.StepLog, session, request.AnchorSelector, request.TimeoutMs, out AutomationElement anchor, out RpcError? resolveError))
        {
            result.Ok = false;
        ...
        if (rect.Width <= 0 || rect.Height <= 0)
        {
            RpcError error = new RpcError
            {
                ...
                Message = "Anchor bounding rectangle is empty",
            };

            StepLogEntry invalidRect = StartStep(result.StepLog, stepId: "ClickRel", action: "ClickRel", selector: request.AnchorSelector);
            MarkFailure(invalidRect, error);
            result.Ok = false;
            result.Error = error;
            return result;
        }

        int x = rect.Left + (int)Math.Round(rect.Width * request.XRatio);
        int y = rect.Top + (int)Math.Round(rect.Height * request.YRatio);

        StepLogEntry clickStep = StartStep(
            result.StepLog,
            stepId: "ClickRel",
            action: "ClickRel",
            selector: request.AnchorSelector,
            parameters: new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ["xRatio"] = request.XRatio.ToString("0.####", System.Globalization.CultureInfo.InvariantCulture),
                ["yRatio"] = request.YRatio.ToString("0.####", System.Globalization.CultureInfo.InvariantCulture),
                ["x"] = x.ToString(),
                ["y"] = y.ToString(),
        ...
        try
        {
            ...
            MarkSuccess(clickStep);
            result.Ok = true;
            return result;
        }
        catch (Exception ex)
        {
            return Fail(result, clickStep, RpcErrorKinds.ActionError, "ClickRel failed", ex);
        }
    }
        ...
        out RpcError? error)
    {
        ...
        StepLogEntry resolveStep = StartStep(
            stepLog,
            stepId: "ResolveAnchor",
            action: "Resolve anchor",
            selector: selector,
            parameters: new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ["timeoutMs"] = timeoutMs.ToString(),
                ["roots"] = "MainWindow->Desktop",
            });

        Window mainWindow;
        try
        {
            mainWindow = session.GetMainWindow(TimeSpan.FromMilliseconds(Math.Min(2_000, timeoutMs)));
        }
        catch (Exception ex)
        {
            error = CreateError(RpcErrorKinds.ConfigError, "Failed to get main window", ex);
        ...
        bool ok = Waiter.PollUntil(
            predicate: () =>
            {
                if (ElementFinder.TryFind(mainWindow, session.Automation, selector, out AutomationElement? e, out string? fk, out Dictionary<string, string>? det) && e is not null)
                {
                    found = e;
                    return true;
                }
                ...
                if (ElementFinder.TryFind(desktop, session.Automation, selector, out AutomationElement? e2, out string? fk2, out Dictionary<string, string>? det2) && e2 is not null)
                {
                    found = e2;
                    return true;
                }
    ...
    private static RpcResult PerformAnchorOffsetClick(
        ...
        MouseButton button)
    {
        ...
        if (rect.Width <= 0 || rect.Height <= 0)
        {
            ...
            StepLogEntry invalidRect = StartStep(result.StepLog, stepId: stepId, action: action, selector: request.AnchorSelector);
            MarkFailure(invalidRect, error);
            result.Ok = false;
            result.Error = error;
            return result;
        ...
        {
            return Fail(result, clickStep, RpcErrorKinds.ActionError, $"{action} failed", ex);
        }
    }
