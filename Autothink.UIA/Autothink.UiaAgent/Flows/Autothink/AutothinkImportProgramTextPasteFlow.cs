...
internal sealed class AutothinkImportProgramTextPasteFlow : IFlow
{
    ...
    private const string StepActionClickAt = "ClickAt";
    private const string StepActionRightClickAt = "RightClickAt";
    private const string StepActionClickRel = "ClickRel";
    private const string StepActionSetText = "SetText";
    private const string StepActionSendKeys = "SendKeys";
    private const string StepActionKeyNav = "KeyNav";
    ...
    private const int DefaultKeyNavIntervalMs = 80;
        ...
        out RpcError? error)
    {
        ...
        ElementSelector? actionSelector = stepAction.AnchorSelector ?? stepAction.Selector;
        ...
        if (actionKind == StepActionKeyNav)
        {
            if (stepAction.KeySequence is null || stepAction.KeySequence.Count == 0)
            {
                error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "KeyNav action requires keySequence" };
                context.MarkFailure(step, error);
                return false;
            }

            int intervalMs = stepAction.IntervalMs.GetValueOrDefault(DefaultKeyNavIntervalMs);
            if (intervalMs < 0)
            {
                error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "KeyNav intervalMs must be >= 0" };
                context.MarkFailure(step, error);
                return false;
            }

            step.Parameters ??= new Dictionary<string, string>(StringComparer.Ordinal);
            step.Parameters["seqLen"] = stepAction.KeySequence.Count.ToString();
            step.Parameters["intervalMs"] = intervalMs.ToString();
            step.Parameters["keys"] = string.Join(",", stepAction.KeySequence);

            try
            {
                for (int i = 0; i < stepAction.KeySequence.Count; i++)
                {
                    string key = stepAction.KeySequence[i];
                    if (!SendKeysParser.TryParse(key, out ParsedSendKeys? parsedKeys, out string? parseError) || parsedKeys is null)
                    {
                        error = new RpcError
                        {
                            Kind = RpcErrorKinds.InvalidArgument,
                            Message = "Failed to parse key sequence",
                            Details = new Dictionary<string, string>(StringComparer.Ordinal)
                            {
                                ["error"] = parseError ?? string.Empty,
                                ["key"] = key,
                    ...
                    switch (parsedKeys.Kind)
                    {
                        ...
                        case ParsedSendKeysKinds.Chord:
                            ...
                            for (int j = parsedKeys.Modifiers.Length - 1; j >= 0; j--)
                            {
                                Keyboard.Release(parsedKeys.Modifiers[j]);
                            }
                    ...
                    if (intervalMs > 0 && i < stepAction.KeySequence.Count - 1)
                    {
            ...
            {
                error = CreateError(RpcErrorKinds.ActionError, "KeyNav failed", ex);
                context.MarkFailure(step, error);
                return false;
            }
        }
        ...
        if (actionKind == StepActionClickAt || actionKind == StepActionRightClickAt || actionKind == StepActionClickRel)
        {
            ElementSelector? anchorSelector = stepAction.AnchorSelector;
            if (anchorSelector is null || anchorSelector.Path is null || anchorSelector.Path.Count == 0)
            {
                error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "AnchorSelector must be provided for anchor action" };
                context.MarkFailure(step, error);
                return false;
            }
            ...
            if (!TryFindElementWithSearchRoot(
                    ...
                    out Dictionary<string, string>? anchorDetails) ||
                anchor is null)
            {
                error = MapFindFailureToError(anchorFailure, anchorDetails);
                context.MarkFailure(step, error);
                return false;
            }

            System.Drawing.Rectangle rect = anchor.BoundingRectangle;
            if (rect.Width <= 0 || rect.Height <= 0)
            {
                error = new RpcError { Kind = RpcErrorKinds.ActionError, Message = "Anchor bounding rectangle is empty" };
                context.MarkFailure(step, error);
                return false;
            }
            ...
            int x;
            int y;
            if (actionKind == StepActionClickRel)
            {
                double xRatio = stepAction.XRatio ?? double.NaN;
                double yRatio = stepAction.YRatio ?? double.NaN;
                if (double.IsNaN(xRatio) || double.IsNaN(yRatio) || xRatio < 0 || xRatio > 1 || yRatio < 0 || yRatio > 1)
                {
                    error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "XRatio/YRatio must be within [0,1] for ClickRel" };
                    context.MarkFailure(step, error);
                    return false;
                }

                x = rect.Left + (int)Math.Round(rect.Width * xRatio);
                y = rect.Top + (int)Math.Round(rect.Height * yRatio);
                step.Parameters ??= new Dictionary<string, string>(StringComparer.Ordinal);
                step.Parameters["xRatio"] = xRatio.ToString("0.####", System.Globalization.CultureInfo.InvariantCulture);
                step.Parameters["yRatio"] = yRatio.ToString("0.####", System.Globalization.CultureInfo.InvariantCulture);
            }
            else
            {
                int offsetX = stepAction.OffsetX ?? 0;
                int offsetY = stepAction.OffsetY ?? 0;
                x = rect.Left + offsetX;
                y = rect.Top + offsetY;
                step.Parameters ??= new Dictionary<string, string>(StringComparer.Ordinal);
                step.Parameters["offsetX"] = offsetX.ToString();
                step.Parameters["offsetY"] = offsetY.ToString();
            }

            step.Parameters ??= new Dictionary<string, string>(StringComparer.Ordinal);
            step.Parameters["x"] = x.ToString();
            step.Parameters["y"] = y.ToString();
            step.Parameters["rect"] = $"{rect.Left},{rect.Top},{rect.Width},{rect.Height}";
            step.Parameters["timeoutMs"] = findTimeoutMs.ToString();
            AddRootParameters(step.Parameters, searchRootKind);

            try
            {
                MouseButton button = actionKind == StepActionRightClickAt ? MouseButton.Right : MouseButton.Left;
                Mouse.Click(new System.Drawing.Point(x, y), button);
                context.MarkSuccess(step);
                error = null;
                return true;
            }
            ...
            {
                error = CreateError(RpcErrorKinds.ActionError, "Anchor click failed", ex);
                context.MarkFailure(step, error);
                return false;
            }
        }
    ...
    private static string? NormalizeActionKind(string? action)
    {
        ...
        if (string.Equals(action, StepActionClickAt, StringComparison.OrdinalIgnoreCase))
        {
            return StepActionClickAt;
        }

        if (string.Equals(action, StepActionRightClickAt, StringComparison.OrdinalIgnoreCase))
        {
            return StepActionRightClickAt;
        }

        if (string.Equals(action, StepActionClickRel, StringComparison.OrdinalIgnoreCase))
        {
            return StepActionClickRel;
        }
        ...
        if (string.Equals(action, StepActionKeyNav, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(action, "key_nav", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(action, "keynav", StringComparison.OrdinalIgnoreCase))
        {
            return StepActionKeyNav;
        }
    ...
    internal static bool TryParseArgs(JsonElement? args, out AutothinkImportProgramTextPasteArgs parsed, out RpcError? error)
    {
        ...
        if (parsed.OpenProgramSteps is { Count: > 0 })
        {
            foreach (ImportProgramStepAction step in parsed.OpenProgramSteps)
            {
                ...
                if (actionKind == StepActionKeyNav)
                {
                    if (step.KeySequence is null || step.KeySequence.Count == 0)
                    {
                        error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "KeyNav action requires keySequence" };
                        return false;
                    }
                ...
                bool isAnchorAction = actionKind == StepActionClickAt || actionKind == StepActionRightClickAt || actionKind == StepActionClickRel;
                if (isAnchorAction)
                {
                    if (step.AnchorSelector is null || step.AnchorSelector.Path is null || step.AnchorSelector.Path.Count == 0)
                    {
                        error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "AnchorSelector must be provided for anchor action" };
                        return false;
                    }
                    ...
                    if (actionKind == StepActionClickRel)
                    {
                        if (step.XRatio is null || step.YRatio is null || step.XRatio < 0 || step.XRatio > 1 || step.YRatio < 0 || step.YRatio > 1)
                        {
                            error = new RpcError { Kind = RpcErrorKinds.InvalidArgument, Message = "XRatio/YRatio must be within [0,1] for ClickRel" };
                            return false;
                        }
                ...
                else if (actionKind != StepActionSendKeys && actionKind != StepActionKeyNav && actionKind != StepActionWaitUntil)
                {
