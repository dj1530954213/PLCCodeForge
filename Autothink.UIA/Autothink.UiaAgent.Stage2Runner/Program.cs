...
internal static class Program
{
    private static async Task<int> Main(string[] args)
    {
        ...
        if (options.ProfileCheck)
        {
            if (options.Demo)
            {
                Console.Error.WriteLine("ProfileCheck mode cannot be combined with --demo.");
                return 2;
            }

            if (config is null)
            {
                Console.Error.WriteLine("ProfileCheck mode requires --config <path>.");
                return 2;
            }
            ...
            return RunProfileCheck(selectorsRoot, profile, logsRoot, config);
        }
        ...
        if (options.Calibrate)
        {
            if (options.Demo)
            {
                Console.Error.WriteLine("Calibrate mode cannot be combined with --demo.");
                return 2;
            }

            if (config is null)
            {
                Console.Error.WriteLine("Calibrate mode requires --config <path>.");
                return 2;
            }
            ...
            return await RunCalibrateAsync(agentExe, config, selectorsRoot, profile, logsRoot, options);
        }
    ...
    private static int RunProfileCheck(
        ...
        RunnerConfig config)
    {
        ...
        Dictionary<string, SelectorProfileLoadResult> profilesForCheck;
        try
        {
            profilesForCheck = new Dictionary<string, SelectorProfileLoadResult>(StringComparer.Ordinal)
            {
                ["autothink.attach"] = LoadProfileWithOverrides(selectorsRoot, profile, "autothink.attach", packVersion),
                ["autothink.importVariables"] = LoadProfileWithOverrides(selectorsRoot, profile, "autothink.importVariables", packVersion),
                ["autothink.importProgram.textPaste"] = LoadProfileWithOverrides(selectorsRoot, profile, "autothink.importProgram.textPaste", packVersion),
                ["autothink.build"] = LoadProfileWithOverrides(selectorsRoot, profile, "autothink.build", packVersion),
                ["autothink.popups"] = LoadProfileWithOverrides(selectorsRoot, profile, "autothink.popups", packVersion),
            };
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"Profile check load failed: {ex.Message}");
            return 6;
        }

        ProfileCheckResult check = RunProfileCheck(runDir, packVersion, profile, config, profilesForCheck);
        Console.WriteLine($"ProfileCheck: {check.ReportPath}");
        return check.Ok ? 0 : 6;
    }

    private static async Task<int> RunCalibrateAsync(
        string agentExe,
        RunnerConfig config,
        string selectorsRoot,
        string profile,
        ...
        RunnerOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.CalibrateFlow))
        {
            Console.Error.WriteLine("Calibrate requires --calibrateFlow <flowName>.");
            return 5;
        }

        if (string.IsNullOrWhiteSpace(options.CalibrateAnchorKey))
        {
            Console.Error.WriteLine("Calibrate requires --anchorKey <anchorKey>.");
            return 5;
        }

        string flowName = options.CalibrateFlow.Trim();
        string anchorKey = options.CalibrateAnchorKey.Trim();
        string? normalizedRoot = NormalizeProbeRoot(options.CalibrateRoot);
        if (normalizedRoot is null)
        {
            Console.Error.WriteLine("Calibrate root must be mainWindow/desktop.");
            return 5;
        }
        ...
        SelectorProfileLoadResult profileResult;
        try
        {
            profileResult = LoadProfileWithOverrides(selectorsRoot, profile, flowName, packVersion);
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"Calibrate profile load failed: {ex.Message}");
            return 6;
        }

        if (!profileResult.Profile.Anchors.TryGetValue(anchorKey, out ProfileAnchor? anchor) || anchor is null)
        {
            Console.Error.WriteLine($"Anchor key not found: {anchorKey}");
            return 6;
        }

        ElementSelector anchorSelector;
        try
        {
            anchorSelector = ResolveAnchorSelector(anchor, profileResult.Profile, $"anchor:{anchorKey}");
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"Resolve anchor selector failed: {ex.Message}");
            return 6;
        }
        ...
        if (openError is null && sessionResult?.Value is not null)
        {
            ...
            {
                Console.Error.WriteLine($"Calibrate attach failed: {attachError}");
                openError = new RpcError { Kind = RpcErrorKinds.ConfigError, Message = attachError ?? "Attach failed" };
            }
        }
        ...
        try
        {
            ...
            {
                Console.Error.WriteLine($"Calibrate open session failed: {openError.Message}");
                return 6;
            }

            if (automation is null || mainWindow is null)
            {
                Console.Error.WriteLine("Calibrate attach failed: automation/mainWindow not available.");
                return 6;
            }

            AutomationElement rootElement = string.Equals(normalizedRoot, "desktop", StringComparison.OrdinalIgnoreCase)
                ? automation.GetDesktop()
                : mainWindow;

            ProbeMatchResult match = EvaluateSelector(rootElement, anchorSelector);
            if (!match.Ok || match.Element is null)
            {
                Console.Error.WriteLine($"Anchor selector not found: {match.FailureMessage}");
                return 6;
            }

            System.Drawing.Rectangle rect = match.Element.BoundingRectangle;
            if (rect.Width <= 0 || rect.Height <= 0)
            {
                Console.Error.WriteLine("Anchor bounding rectangle is empty.");
                return 6;
            }

            double scale = profileResult.Profile.Metadata?.DpiScale ?? 1.0;
            if (scale <= 0)
            {
                scale = 1.0;
            }
            ...
            int anchorOffsetX = 0;
            int anchorOffsetY = 0;
            if (anchor.Offset is { Length: >= 2 })
            {
                anchorOffsetX = (int)Math.Round(anchor.Offset[0] * scale);
                anchorOffsetY = (int)Math.Round(anchor.Offset[1] * scale);
            }

            Console.WriteLine($"Anchor: {anchorKey}");
            Console.WriteLine($"Root: {normalizedRoot}");
            Console.WriteLine($"Rect: {rect.Left},{rect.Top},{rect.Width},{rect.Height}");
            Console.WriteLine($"DpiScale: {scale:0.###}");
            Console.WriteLine($"AnchorOffset(px): {anchorOffsetX},{anchorOffsetY}");

            var positions = new Dictionary<string, ProfilePosition>(StringComparer.Ordinal);

            if (!string.IsNullOrWhiteSpace(options.CalibratePositionKey))
            {
                string key = options.CalibratePositionKey.Trim();
                if (!CapturePosition(key, rect, anchorKey, anchorOffsetX, anchorOffsetY, scale, positions))
                {
                    return 6;
                }
            }
            else
            {
                while (true)
                {
                    Console.Write("PositionKey (empty to finish): ");
                    ...
                    {
                        Console.WriteLine($"PositionKey already exists: {key}");
                        continue;
                    }
            ...
            {
                Console.Error.WriteLine("No positions captured.");
                return 6;
            }

            string outputPath = ResolveOutputPath(options.CalibrateOutput, runDir, "calibration_positions.json");
            var output = new CalibrationOutput
            {
                AnchorKey = anchorKey,
                AnchorSelectorKey = anchor.SelectorKey,
                AnchorRect = new[] { rect.Left, rect.Top, rect.Width, rect.Height },
            ...
            Console.WriteLine($"Calibration output: {outputPath}");
            return 0;
        }
        finally
        {
    ...
    private static bool CapturePosition(
        string key,
        System.Drawing.Rectangle anchorRect,
        string anchorKey,
        int anchorOffsetX,
        int anchorOffsetY,
        double scale,
        Dictionary<string, ProfilePosition> positions)
    {
        Console.WriteLine($"Move mouse to target for '{key}', then press Enter to capture.");
        _ = Console.ReadLine();

        if (!GetCursorPos(out NativePoint cursor))
        {
            Console.Error.WriteLine("GetCursorPos failed.");
            return false;
        }

        int dx = cursor.X - anchorRect.Left - anchorOffsetX;
        int dy = cursor.Y - anchorRect.Top - anchorOffsetY;
        int posX = (int)Math.Round(dx / scale);
        int posY = (int)Math.Round(dy / scale);

        double ratioX = anchorRect.Width > 0 ? (double)(cursor.X - anchorRect.Left) / anchorRect.Width : 0;
        double ratioY = anchorRect.Height > 0 ? (double)(cursor.Y - anchorRect.Top) / anchorRect.Height : 0;

        positions[key] = new ProfilePosition
        {
            Anchor = anchorKey,
            Point = new[] { posX, posY },
        };

        Console.WriteLine($"Captured {key}: screen=({cursor.X},{cursor.Y}) rel=({posX},{posY}) ratio=({ratioX:0.####},{ratioY:0.####})");
        return true;
    }
        ...
        string? packVersion)
    {
        ...
        if (File.Exists(baseBaselinePath))
        {
            ...
            MergeAnchors(merged.Anchors, baseProfile.Anchors, overwrite: true);
            MergePositions(merged.Positions, baseProfile.Positions, overwrite: true);
            MergeNavSequences(merged.NavSequences, baseProfile.NavSequences, overwrite: true);
            MergeMetadata(merged, baseProfile);
        ...
        if (File.Exists(baseLocalPath))
        {
            ...
            MergeAnchors(merged.Anchors, baseLocal.Anchors, overwrite: true);
            MergePositions(merged.Positions, baseLocal.Positions, overwrite: true);
            MergeNavSequences(merged.NavSequences, baseLocal.NavSequences, overwrite: true);
            MergeMetadata(merged, baseLocal);
            usedLocal = true;
        ...
        MergeAnchors(merged.Anchors, baseline.Anchors, overwrite: false);
        MergePositions(merged.Positions, baseline.Positions, overwrite: false);
        MergeNavSequences(merged.NavSequences, baseline.NavSequences, overwrite: false);
        MergeMetadata(merged, baseline);
        ...
        if (File.Exists(localPath))
        {
            ...
            MergeAnchors(merged.Anchors, local.Anchors, overwrite: true);
            MergePositions(merged.Positions, local.Positions, overwrite: true);
            MergeNavSequences(merged.NavSequences, local.NavSequences, overwrite: true);
            MergeMetadata(merged, local);
            usedLocal = true;
    ...
    private static SelectorProfileFile LoadProfileFile(string path)
    {
        ...
        if (profileFile.Anchors is null)
        {
            profileFile.Anchors = new Dictionary<string, ProfileAnchor>(StringComparer.Ordinal);
        }

        if (profileFile.Positions is null)
        {
            profileFile.Positions = new Dictionary<string, ProfilePosition>(StringComparer.Ordinal);
        }

        if (profileFile.NavSequences is null)
        {
            profileFile.NavSequences = new Dictionary<string, ProfileNavSequence>(StringComparer.Ordinal);
        }
    ...
    private static void MergeAnchors(
        Dictionary<string, ProfileAnchor> target,
        Dictionary<string, ProfileAnchor> source,
        bool overwrite)
    {
        foreach (KeyValuePair<string, ProfileAnchor> kv in source)
        {
            if (overwrite || !target.ContainsKey(kv.Key))
            {
                target[kv.Key] = kv.Value;
    ...
    private static void MergePositions(
        Dictionary<string, ProfilePosition> target,
        Dictionary<string, ProfilePosition> source,
        bool overwrite)
    {
        foreach (KeyValuePair<string, ProfilePosition> kv in source)
        {
            if (overwrite || !target.ContainsKey(kv.Key))
            {
                target[kv.Key] = kv.Value;
    ...
    private static void MergeNavSequences(
        Dictionary<string, ProfileNavSequence> target,
        Dictionary<string, ProfileNavSequence> source,
        bool overwrite)
    {
        foreach (KeyValuePair<string, ProfileNavSequence> kv in source)
        {
            if (overwrite || !target.ContainsKey(kv.Key))
            {
                target[kv.Key] = kv.Value;
    ...
    private static void MergeMetadata(SelectorProfileFile target, SelectorProfileFile source)
    {
        ...
        target.Metadata ??= new ProfileMetadata();
        ProfileMetadata t = target.Metadata;
        ProfileMetadata s = source.Metadata;

        t.App = string.IsNullOrWhiteSpace(t.App) ? s.App : t.App;
        t.Version = string.IsNullOrWhiteSpace(t.Version) ? s.Version : t.Version;
        t.Lang = string.IsNullOrWhiteSpace(t.Lang) ? s.Lang : t.Lang;
        t.Resolution = string.IsNullOrWhiteSpace(t.Resolution) ? s.Resolution : t.Resolution;
        t.DpiScale = t.DpiScale ?? s.DpiScale;
    }
    ...
    private static ResolvedPosition ResolvePosition(SelectorProfileFile profile, string positionKey, string context)
    {
        if (string.IsNullOrWhiteSpace(positionKey))
        {
            throw new InvalidOperationException($"Position key is required: {context}");
        }

        if (!profile.Positions.TryGetValue(positionKey, out ProfilePosition? position) || position is null)
        {
            throw new KeyNotFoundException($"Position key not found: {positionKey} ({context})");
        }

        if (string.IsNullOrWhiteSpace(position.Anchor))
        {
            throw new InvalidOperationException($"Position.anchor is required: {positionKey} ({context})");
        }

        if (!profile.Anchors.TryGetValue(position.Anchor, out ProfileAnchor? anchor) || anchor is null)
        {
            throw new KeyNotFoundException($"Anchor key not found: {position.Anchor} (position={positionKey}, {context})");
        }

        double scale = profile.Metadata?.DpiScale ?? 1.0;
        if (scale <= 0)
        {
            scale = 1.0;
        }

        ElementSelector anchorSelector = ResolveAnchorSelector(anchor, profile, $"anchor:{position.Anchor} ({context})");
        int offsetX;
        int offsetY;

        if (position.Point is { Length: >= 2 })
        {
            offsetX = position.Point[0];
            offsetY = position.Point[1];
        }
        else if (position.Rect is { Length: >= 4 })
        {
            int x = position.Rect[0];
            int y = position.Rect[1];
            int w = position.Rect[2];
            int h = position.Rect[3];
            offsetX = x + (w / 2);
            offsetY = y + (h / 2);
        }
        else
        {
            throw new InvalidOperationException($"Position must define point or rect: {positionKey} ({context})");
        }

        offsetX = (int)Math.Round(offsetX * scale);
        offsetY = (int)Math.Round(offsetY * scale);

        if (anchor.Offset is { Length: >= 2 })
        {
            offsetX += (int)Math.Round(anchor.Offset[0] * scale);
            offsetY += (int)Math.Round(anchor.Offset[1] * scale);
        }
        ...
        return new ResolvedPosition
        {
            ...
            OffsetX = offsetX,
            OffsetY = offsetY,
            AnchorKey = position.Anchor,
            PositionKey = positionKey,
        };
    }

    private static ElementSelector ResolveAnchorSelector(ProfileAnchor anchor, SelectorProfileFile profile, string context)
    {
        string type = string.IsNullOrWhiteSpace(anchor.Type) ? "selector" : anchor.Type.Trim();
        if (!string.Equals(type, "selector", StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException($"Anchor type '{type}' is not supported: {context}");
        }

        if (anchor.Selector is not null && anchor.Selector.Path.Count > 0)
        {
            return anchor.Selector;
        }

        if (!string.IsNullOrWhiteSpace(anchor.SelectorKey))
        {
            return ResolveSelector(null, anchor.SelectorKey, profile, context);
        }

        throw new InvalidOperationException($"Anchor selector is required: {context}");
    }
        ...
        SelectorProfileFile profile)
    {
        ...
        {
            ImportDialogStepConfig step = steps[i];
            ElementSelector? selector = ResolveSelectorOptional(step.Selector, step.SelectorKey, profile);
            ElementSelector? anchorSelector = ResolveSelectorOptional(step.AnchorSelector, step.AnchorSelectorKey, profile);
            int? offsetX = step.OffsetX;
            int? offsetY = step.OffsetY;
            double? xRatio = step.XRatio;
            double? yRatio = step.YRatio;
            List<string>? keySequence = step.KeySequence;
            int? intervalMs = step.IntervalMs;
            WaitCondition? condition = ResolveWaitCondition(step.Condition, profile);

            if (!string.IsNullOrWhiteSpace(step.PositionKey))
            {
                ResolvedPosition resolved = ResolvePosition(profile, step.PositionKey, $"dialogSteps[{i}]");
                anchorSelector = resolved.AnchorSelector;
                offsetX = resolved.OffsetX;
                offsetY = resolved.OffsetY;
            }

            if (!string.IsNullOrWhiteSpace(step.NavSequenceKey))
            {
                if (string.IsNullOrWhiteSpace(step.Action) ||
                    !(string.Equals(step.Action, "KeyNav", StringComparison.OrdinalIgnoreCase) ||
                      string.Equals(step.Action, "key_nav", StringComparison.OrdinalIgnoreCase) ||
                      string.Equals(step.Action, "keynav", StringComparison.OrdinalIgnoreCase)))
                {
                    throw new InvalidOperationException($"dialogSteps[{i}].action must be KeyNav when navSequenceKey is set.");
                }

                if (!profile.NavSequences.TryGetValue(step.NavSequenceKey, out ProfileNavSequence? nav) || nav is null)
                {
                    throw new KeyNotFoundException($"NavSequence key not found: {step.NavSequenceKey} (dialogSteps[{i}])");
                }

                if (nav.Keys is null || nav.Keys.Count == 0)
                {
                    throw new InvalidOperationException($"NavSequence keys empty: {step.NavSequenceKey} (dialogSteps[{i}])");
                }

                keySequence = nav.Keys;
                intervalMs ??= nav.IntervalMs;
            }
    ...
    private static string ResolveOutputPath(string? path, string baseDir, string defaultFileName)
    {
        if (string.IsNullOrWhiteSpace(path))
        {
    ...
    private static string WriteProfileCheckReport(string runDir, ProfileCheckReport report)
    {
        string path = Path.Combine(runDir, "profile_check_report.json");
        string json = JsonSerializer.Serialize(report, JsonOptions);
        File.WriteAllText(path, json, Encoding.UTF8);
        return path;
    }
    ...
    private static ProfileCheckResult RunProfileCheck(
        string runDir,
        string? packVersion,
        string profileName,
        RunnerConfig config,
        IReadOnlyDictionary<string, SelectorProfileLoadResult> profiles)
    {
        ProfileCheckReport report = BuildProfileCheckReport(packVersion, profileName, config, profiles);
        string reportPath = WriteProfileCheckReport(runDir, report);
        bool ok = report.Flows.All(flow =>
            flow.MissingPositions.Count == 0 &&
            flow.MissingNavSequences.Count == 0 &&
            flow.InvalidAnchors.Count == 0 &&
            flow.InvalidPositions.Count == 0 &&
            flow.InvalidNavSequences.Count == 0);
    ...
    private static ProfileCheckReport BuildProfileCheckReport(
        string? packVersion,
        string profileName,
        RunnerConfig config,
        IReadOnlyDictionary<string, SelectorProfileLoadResult> profiles)
    {
        Dictionary<string, ProfileUsageRequirements> requirements = BuildProfileUsageRequirements(config);
        var flowReports = new List<ProfileCheckFlowReport>(profiles.Count);

        foreach ((string flowName, SelectorProfileLoadResult profileResult) in profiles)
        {
            SelectorProfileFile profile = profileResult.Profile;
            ProfileUsageRequirements usage = requirements.TryGetValue(flowName, out ProfileUsageRequirements? req)
                ? req
                : new ProfileUsageRequirements();

            var flowReport = new ProfileCheckFlowReport
            {
                FlowName = flowName,
                RequiredPositions = usage.PositionKeys.OrderBy(k => k, StringComparer.Ordinal).ToList(),
                RequiredNavSequences = usage.NavSequenceKeys.OrderBy(k => k, StringComparer.Ordinal).ToList(),
            };

            foreach (string key in usage.PositionKeys)
            {
                if (!profile.Positions.ContainsKey(key))
                {
                    flowReport.MissingPositions.Add(key);
                }
            }

            foreach (string key in usage.NavSequenceKeys)
            {
                if (!profile.NavSequences.ContainsKey(key))
                {
                    flowReport.MissingNavSequences.Add(key);
                }
            }

            ValidateProfileDefinitions(profile, flowReport.InvalidAnchors, flowReport.InvalidPositions, flowReport.InvalidNavSequences);
    ...
    private static Dictionary<string, ProfileUsageRequirements> BuildProfileUsageRequirements(RunnerConfig config)
    {
        var requirements = new Dictionary<string, ProfileUsageRequirements>(StringComparer.Ordinal);

        ProfileUsageRequirements importVariables = CollectDialogStepRequirements(config.ImportVariables?.OpenImportDialogSteps);
        if (importVariables.HasAny)
        {
            requirements["autothink.importVariables"] = importVariables;
        }

        ProfileUsageRequirements importProgram = CollectDialogStepRequirements(config.ImportProgram?.OpenProgramSteps);
        if (importProgram.HasAny)
        {
            requirements["autothink.importProgram.textPaste"] = importProgram;
        }

        return requirements;
    }

    private static ProfileUsageRequirements CollectDialogStepRequirements(IReadOnlyList<ImportDialogStepConfig>? steps)
    {
        var requirements = new ProfileUsageRequirements();
        if (steps is null || steps.Count == 0)
        {
            return requirements;
        }
        ...
        foreach (ImportDialogStepConfig step in steps)
        {
            if (!string.IsNullOrWhiteSpace(step.PositionKey))
            {
                requirements.PositionKeys.Add(step.PositionKey.Trim());
            }

            if (!string.IsNullOrWhiteSpace(step.NavSequenceKey))
            {
                requirements.NavSequenceKeys.Add(step.NavSequenceKey.Trim());
            }
        }
    ...
    private static void ValidateProfileDefinitions(
        SelectorProfileFile profile,
        List<string> invalidAnchors,
        List<string> invalidPositions,
        List<string> invalidNavSequences)
    {
        foreach ((string key, ProfileAnchor anchor) in profile.Anchors)
        {
            string type = string.IsNullOrWhiteSpace(anchor.Type) ? "selector" : anchor.Type.Trim();
            bool hasInlineSelector = anchor.Selector is not null && anchor.Selector.Path.Count > 0;
            bool hasSelectorKey = !string.IsNullOrWhiteSpace(anchor.SelectorKey);

            if (!string.Equals(type, "selector", StringComparison.OrdinalIgnoreCase))
            {
                invalidAnchors.Add(key);
                continue;
            }

            if (!hasInlineSelector && !hasSelectorKey)
            {
                invalidAnchors.Add(key);
                continue;
            }

            if (hasSelectorKey && !profile.Selectors.ContainsKey(anchor.SelectorKey!))
            {
                invalidAnchors.Add(key);
            }
        }

        foreach ((string key, ProfilePosition position) in profile.Positions)
        {
            bool hasPoint = position.Point is { Length: >= 2 };
            bool hasRect = position.Rect is { Length: >= 4 };
            ...
            if (string.IsNullOrWhiteSpace(position.Anchor) || !profile.Anchors.ContainsKey(position.Anchor))
            {
                invalidPositions.Add(key);
            }
        }

        foreach ((string key, ProfileNavSequence navSequence) in profile.NavSequences)
        {
            if (navSequence.Keys is null || navSequence.Keys.Count == 0)
            {
                invalidNavSequences.Add(key);
            }
        }
    }
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  --profileCheck         Run profile check mode (requires --config)");
        Console.WriteLine("  --calibrate            Run calibration mode (requires --config)");
        Console.WriteLine("  --calibrateFlow <flow> Flow name for calibration (e.g. autothink.importVariables)");
        Console.WriteLine("  --anchorKey <key>       Anchor key to calibrate");
        Console.WriteLine("  --positionKey <key>     Position key for single capture (optional)");
        Console.WriteLine("  --calibrateRoot <r>     mainWindow/desktop (default: mainWindow)");
        Console.WriteLine("  --calibrateOutput <p>   Output json path (default: runDir/calibration_positions.json)");
    ...
    private sealed class ProfileCheckResult
    {
        public bool Ok { get; set; }

        public ProfileCheckReport Report { get; set; } = new();
    ...
    private sealed class ProfileCheckReport
    {
        public string PackVersion { get; set; } = "default";

        public string Profile { get; set; } = string.Empty;

        public IReadOnlyList<ProfileCheckFlowReport> Flows { get; set; } = Array.Empty<ProfileCheckFlowReport>();
    ...
    private sealed class CalibrationOutput
    {
        public string AnchorKey { get; set; } = string.Empty;

        public string? AnchorSelectorKey { get; set; }

        public int[] AnchorRect { get; set; } = Array.Empty<int>();

        public int[]? AnchorOffset { get; set; }

        public double DpiScale { get; set; } = 1.0;

        public Dictionary<string, ProfilePosition> Positions { get; set; } = new(StringComparer.Ordinal);

        public DateTimeOffset GeneratedAtUtc { get; set; }
    }

    private sealed class ProfileCheckFlowReport
    {
        public string FlowName { get; set; } = string.Empty;

        public List<string> RequiredPositions { get; set; } = new();

        public List<string> MissingPositions { get; set; } = new();

        public List<string> RequiredNavSequences { get; set; } = new();

        public List<string> MissingNavSequences { get; set; } = new();

        public List<string> InvalidAnchors { get; set; } = new();

        public List<string> InvalidPositions { get; set; } = new();

        public List<string> InvalidNavSequences { get; set; } = new();
    }

    private sealed class ProfileUsageRequirements
    {
        public HashSet<string> PositionKeys { get; } = new(StringComparer.Ordinal);

        public HashSet<string> NavSequenceKeys { get; } = new(StringComparer.Ordinal);

        public bool HasAny => PositionKeys.Count > 0 || NavSequenceKeys.Count > 0;
    }
    ...
    private sealed class RunnerOptions
    {
        ...
        public string? CalibrateFlow { get; set; }

        public string? CalibrateAnchorKey { get; set; }

        public string? CalibratePositionKey { get; set; }

        public string? CalibrateOutput { get; set; }

        public string? CalibrateRoot { get; set; }
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                switch (arg)
                {
                    ...
                    case "--calibrate":
                        ...
                        break;
                    case "--calibrateFlow":
                        options.CalibrateFlow = i + 1 < args.Length ? args[++i] : null;
                        break;
                    case "--anchorKey":
                        options.CalibrateAnchorKey = i + 1 < args.Length ? args[++i] : null;
                        break;
                    case "--positionKey":
                        options.CalibratePositionKey = i + 1 < args.Length ? args[++i] : null;
                        break;
                    case "--calibrateOutput":
                        options.CalibrateOutput = i + 1 < args.Length ? args[++i] : null;
                        break;
                    case "--calibrateRoot":
                        options.CalibrateRoot = i + 1 < args.Length ? args[++i] : null;
                        break;
                    case "--probeFlow":
    ...
    private sealed class SelectorProfileFile
    {
        ...
        public Dictionary<string, ProfileAnchor> Anchors { get; set; } = new(StringComparer.Ordinal);

        public Dictionary<string, ProfilePosition> Positions { get; set; } = new(StringComparer.Ordinal);

        public Dictionary<string, ProfileNavSequence> NavSequences { get; set; } = new(StringComparer.Ordinal);
    }
    ...
    private sealed class ProfileAnchor
    {
    ...
    private sealed class ProfilePosition
    {
    ...
    private sealed class ProfileNavSequence
    {
    ...
    private sealed class ResolvedPosition
    {
        public ElementSelector AnchorSelector { get; set; } = new();
    ...
    private sealed class ImportDialogStepConfig
    {
        ...
        public ElementSelector? AnchorSelector { get; set; }
        ...
        public string? NavSequenceKey { get; set; }
