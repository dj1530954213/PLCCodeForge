...
- **改动清单**：
  - `Autothink.UIA/Autothink.UiaAgent.Stage2Runner/Program.cs`：新增自检流程、方法探测、connectivity.json 落盘、summary 字段扩展。
  - `Autothink.UIA/Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：新增 `--check` 使用说明。
...
- **--check 失败示例（本机 Agent 仅 Ping）**：
  - stdout 片段：
    ```text
    RunDir: C:\Program Files\Git\code\PLCCodeForge\Autothink.UIA\logs\20260103-174245
    Connectivity: C:\Program Files\Git\code\PLCCodeForge\Autothink.UIA\logs\20260103-174245\connectivity.json
    ...
    ```
  - `Autothink.UIA/logs/20260103-174245/connectivity.json` 片段（含 durationMs）：
    ```json
    {
      "ok": false,
      "handshakeReady": true,
...
- **复现与定位 OpenSession not found**：
  - 复现：运行 `dotnet run ... --config Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json`。
...
- **summary.json 片段（connectivityOk/failedReason）**：
  - `Autothink.UIA/logs/20260103-174318/summary.json`
    ```json
    {
