...
## 完成摘要
- Stage2Runner 默认生成 `evidence_pack_v1`，包含 summary/selector_check/build_outcome/step_Autothink.UIA/logs/resolved_inputs 等证据文件。
- 产出 `evidence_summary.v1.json`（含 digests 与 keyMetrics），并提供 `--verify` 校验命令。
- DemoTarget 全链路跑通并生成证据包；verify 成功与篡改后失败均可复现。

## 改动清单
- `Autothink.UIA/Autothink.UiaAgent.Stage2Runner/Program.cs`：evidence pack 生成、step_logs 汇总、`--verify` 校验。
- `Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json`：新增 `evidencePack` 配置示例。
- `Autothink.UIA/Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：新增 EvidencePack 生成/验证说明。
...
来自 `Autothink.UIA/logs/20260104-004044/evidence_pack_v1`：
```text
build_outcome.json
...
来自 `Autothink.UIA/logs/20260104-004044/evidence_pack_v1/evidence_summary.v1.json`：
```json
{
  "packVersion": "v1",
  "digests": {
...
## verify 成功输出
```text
Stage2Runner.exe --verify --evidence Autothink.UIA/logs/20260104-004044/evidence_pack_v1
Evidence verify: OK
```

## 篡改后 verify 失败输出（digest mismatch）
```text
Stage2Runner.exe --verify --evidence Autothink.UIA/logs/20260104-004044/evidence_pack_v1_tampered
...
- 验证命令：Stage2Runner.exe --verify --evidence Autothink.UIA/logs/<timestamp>/evidence_pack_v1
```

## 自检清单
- [x] 未修改 RPC 契约，仅在 Runner 侧落盘与汇总。
- [x] evidence_pack_v1 默认生成，包含 summary/selector_check/step_Autothink.UIA/logs/build_outcome。
