...
- **改动清单**：
  - `Autothink.UIA/Autothink.UiaAgent/Rpc/Contracts/ElementSelector.cs`：
    - `SelectorStep` 新增 `NameContains`、`IgnoreCase` 字段。
  - `Autothink.UIA/Autothink.UiaAgent/Uia/ElementFinder.cs`：
    - `hasAnyFilter` 逻辑纳入 `NameContains`。
    - Name 匹配改为统一走 `MatchesText(...)`，支持 exact/contains/ignorecase 且默认保持原行为。
  - `Autothink.UIA/Autothink.UiaAgent.Tests/ElementFinderTextMatchTests.cs`：
