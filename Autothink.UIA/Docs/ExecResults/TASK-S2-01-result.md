...
- **完成摘要**：
  ...
  - 已确认 `RpcErrorKinds.NotImplemented` 已存在于 `Autothink.UIA/Autothink.UiaAgent/Rpc/Contracts/RpcError.cs`，无需修改。

- **改动清单**：
  - `Autothink.UIA/Autothink.UiaAgent/Flows/FlowNames.cs`：新增 FlowName 常量与已知集合判断。
  - `Autothink.UIA/Autothink.UiaAgent/Flows/FlowRegistry.cs`：新增注册表骨架与 StubFlow 占位实现。
  - `Autothink.UIA/Autothink.UiaAgent/Flows/FlowDispatcher.cs`：改为从 FlowRegistry 分发，未知/未实现分支按规范返回错误。
  - `Autothink.UIA/Autothink.UiaAgent/Rpc/UiaRpcService.cs`：RunFlow 先按 flowName 返回 InvalidArgument/NotImplemented，再进入 session 解析与 flow 执行。
  - `Autothink.UIA/Autothink.UiaAgent.Tests/RunFlowDispatchTests.cs`：新增单测覆盖“未知/未实现 flow 不依赖 session”语义。
  - `Autothink.UIA/Autothink.UiaAgent/Rpc/Contracts/RpcError.cs`：已包含 `NotImplemented`（仅核对，无代码变更）。
