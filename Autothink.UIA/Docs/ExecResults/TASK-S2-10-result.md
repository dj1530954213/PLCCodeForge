...
- **改动清单**：
  - `Autothink.UIA/Autothink.UiaAgent/Rpc/Contracts/ElementSelector.cs`：新增字段定义。
  - `Autothink.UIA/Autothink.UiaAgent/Uia/ElementFinder.cs`：匹配逻辑与归一化。
  - `Autothink.UIA/Autothink.UiaAgent/Flows/FlowContext.cs`：matchRule 参数注入。
  - `Autothink.UIA/Autothink.UiaAgent/Rpc/UiaRpcService.cs`：matchRule 参数注入。
  - `Autothink.UIA/Autothink.UiaAgent.Tests/ElementFinderTextMatchTests.cs`：新增测试覆盖。
