...
- **改动清单**：
  - `Autothink.UIA/Autothink.UiaAgent/Flows/Autothink/AutothinkImportProgramTextPasteFlow.cs`：新增 verifyMode/afterPasteWaitMs 逻辑，StepId 对齐（SendKeysPaste/VerifyPaste/FallbackTypeText 等）。
  - `Autothink.UIA/Autothink.UiaAgent.Tests/AutothinkImportProgramTextPasteArgsTests.cs`：新增参数校验单测。
  - `Autothink.UIA/Autothink.UiaAgent.Stage2Runner/Program.cs`：示例 args 更新 + JsonElement 安全序列化（便于回归脚本输出）。
...
- **WinFormsHarness 手工验证步骤**：
  1) `dotnet run --project Autothink.UIA/Autothink.UiaAgent.WinFormsHarness/Autothink.UiaAgent.WinFormsHarness.csproj -c Release`
