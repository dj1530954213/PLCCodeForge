...
- 推荐方式：由 Stage2Runner 按 `agentPath` 自动启动（见 `Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json`）。
- 也可手动启动（用于快速验证）：
  - `dotnet run --project Autothink.UIA/Autothink.UiaAgent/Autothink.UiaAgent.csproj -c Release`
...
4. 将 selector 写入 profile 文件：
   - 路径：`Autothink.UIA/Docs/组态软件自动操作/Selectors/*.json`
...
1. 使用 Stage2Runner 探针模式（按 flow + key 校验）：
   - `dotnet build Autothink.UIA/PLCCodeForge.sln -c Release`
   - `dotnet run --project Autothink.UIA/Autothink.UiaAgent.Stage2Runner/Autothink.UiaAgent.Stage2Runner.csproj -c Release --config Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json --probe --probeFlow autothink.build --probeKeys buildButton,buildStatus --probeSearchRoot desktop --probeTimeoutMs 5000`
2. 产物位置：
   - `Autothink.UIA/logs/<timestamp>/probe.<flow>.json`
3. 依据 probe 输出修正：
...
- 真实 AUTOTHINK 模板：`Autothink.UIA/Docs/组态软件自动操作/Selectors/autothink.v1.local.sample.json`
...
### 2.3.1 现场经验（Scintilla 自绘编辑器）

- 【重要】保持 `openProgramSteps` 的最后 `WaitUntil` 仍指向“中间层”即可，不要在 WaitUntil 上强行下钻到最内层。
- 【重要】仅在最终点击与编辑器 selector 上下钻到最内层：`工作区` → `PRG).st` → `Descendants` → `Pane(ClassName=Scintilla, Index=0)`。
- 【重要】不要依赖 `AutomationId`（现场会变），以层级关系 + `ClassName=Scintilla` 为主。
- 成功样例参考：`Autothink.UIA/Docs/组态软件自动操作/FlowSamples/right_click_add_program_and_paste.importProgram.args.json`（当前版本已按上述规则调整）。

## 3. Stage2Runner 一键回归步骤

1. 先做联通性自检（推荐）：
   - 确认 config 中 `agentPath` 指向 FullHost。
   - `dotnet run --project Autothink.UIA/Autothink.UiaAgent.Stage2Runner/Autothink.UiaAgent.Stage2Runner.csproj -c Release --check --config Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json --timeoutMs 2000`
   - 产物：`Autothink.UIA/logs/<timestamp>/connectivity.json`
1. 准备配置文件：
   - 示例：`Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json`（DemoTarget 用例，现场需替换 processName/title）
...
4. 运行输出：
   ...
   - `Autothink.UIA/logs/<timestamp>/build_outcome.json`（编译签收证据）
   - `Autothink.UIA/logs/<timestamp>/summary.json`（快速定位用）
...
- selector 来源：`Autothink.UIA/Docs/组态软件自动操作/Selectors/autothink.popups.json`（可用 `.local.json` 覆盖）。

## 3.4 buildOutcome（编译签收模板）

> 通过 buildOutcome 判定编译成功/失败，产物落盘 `Autothink.UIA/logs/<timestamp>/build_outcome.json`。
...
- 生成位置：`Autothink.UIA/logs/<timestamp>/evidence_pack_v1/`
...
- 验证命令：
  - `Autothink.UiaAgent.Stage2Runner.exe --verify --evidence Autothink.UIA/logs/<timestamp>/evidence_pack_v1`
...
- 产物：
  - `Autothink.UIA/logs/<timestamp>/unexpected_ui_state.json`
...
- 将 `Autothink.UIA/logs/<timestamp>/` 目录整包提交。
- 同时提交：
  - `Autothink.UIA/Docs/组态软件自动操作/Selectors/` 下的 baseline 与 `.local.json`（如有）。
  - 运行时的配置文件（例如 `Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json`）。
