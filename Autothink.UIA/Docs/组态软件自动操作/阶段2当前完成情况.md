...
- Stage2Runner（自动回归）
  ...
  - 读取配置 + profile，顺序跑通 4 条 flow，并输出 StepLog 到 `Autothink.UIA/logs/<timestamp>/`。

## 4. Selector 存放位置

- 统一存放：`Autothink.UIA/Docs/组态软件自动操作/Selectors/`
...
- 按 Runbook 提交 `Autothink.UIA/logs/<timestamp>/` 证据目录。
