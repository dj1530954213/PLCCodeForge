...
- **TASK-S2-12**：Stage2Runner 配置化回归 + StepLog 输出到 `Autothink.UIA/logs/<timestamp>/`。
- **TASK-S2-13**：Runbook 文档已输出。
- **TASK-S2-14**：selector 基线资产包 + local override + summary.json 输出已完成。

## 3. 关键落点

- Selector profile：`Autothink.UIA/Docs/组态软件自动操作/Selectors/`
- 回归配置：`Autothink.UIA/Docs/组态软件自动操作/RunnerConfig/demo.json`
- Runbook：`Autothink.UIA/Docs/组态软件自动操作/Runbook-Autothink-普通型.md`
- Selector baseline/local：`Autothink.UIA/Docs/组态软件自动操作/Selectors/`
...
- 在真实 AUTOTHINK 普通型环境跑通 4 条 flow，提交 `Autothink.UIA/logs/<timestamp>/` 证据。
