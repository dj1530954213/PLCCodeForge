...
import { dtypeRegisterSpan, nextAddress, parseHumanAddress } from "../../src/comm/services/address";
...
describe("comm address", () => {
  ...
  it("advances by dtype span (UInt16 = 1)", () => {
    const start = "40001";
    expect(dtypeRegisterSpan("UInt16")).toBe(1);
  ...
  it("advances by dtype span (Float32 = 2)", () => {
    const start = "40001";
    expect(dtypeRegisterSpan("Float32")).toBe(2);
