...
- `Tauri.CommMapping/src-tauri/src/comm/core/model.rs`：新增 `CommPointsBatchTemplateV1`，并扩展 `CommProjectUiStateV1.pointsBatchTemplate`（可选字段）。
- `Tauri.CommMapping/src-tauri/src/comm/tauri_api.rs`：新增 command：`comm_project_ui_state_patch_v1(projectId, patch)`（只更新 uiState，不改 points/profiles）。
- `Tauri.CommMapping/src-tauri/src/lib.rs`：注册 `comm_project_ui_state_patch_v1`。
