...
- 创建 `Tauri.CommMapping/src-tauri/src/comm/` 目录与空模块文件
...
- 创建 `Tauri.CommMapping/src-tauri/src/comm/` 目录与基础骨架文件（model/codec/plan/driver/engine/export_xlsx/tauri_api），但暂不在对外 API 中暴露具体实现。

**验收证据：**
- `pnpm dev`、`pnpm build`
- `cargo build`
- 文件树片段：包含 `Tauri.CommMapping/src-tauri/src/comm/` 及子模块骨架
