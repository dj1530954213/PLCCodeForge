...
- **完成摘要**：
  ...
  - IT（ENV gating）：
    - `Tauri.CommMapping/src-tauri/tests/comm_it.rs` 在 `COMM_IT_ENABLE=1` 时打印 `job.start/job.len`（便于对照真机寄存器布局）。
...
- **完成证据**：
  - `cargo test --manifest-path Tauri.CommMapping/src-tauri/Cargo.toml`（新增 plan 单测出现在输出中）：
    ```text
