...
- **完成摘要**：
  - 在 `Tauri.CommMapping/src-tauri/src/comm/export_xlsx.rs` 使用 `rust_xlsxwriter` 导出 `通讯地址表.xlsx`，固定输出三张 sheet：`TCP通讯地址表` / `485通讯地址表` / `通讯参数`。
...
- **改动清单**：
  ...
  - `Tauri.CommMapping/Docs/ExecResults/TASK-10-result.md`
    - 新建/更新任务结果归档文件（本文件）
...
- **验收自检**：
  ...
  - [x] `Tauri.CommMapping/Docs/ExecResults/TASK-10-result.md` 已归档
