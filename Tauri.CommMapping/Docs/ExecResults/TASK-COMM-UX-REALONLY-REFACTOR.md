...
| 文件 | 变更类型 | 关键点/理由 |
|---|---|---|
| `Tauri.CommMapping/src-tauri/src/comm/adapters/driver/mock.rs` | 删除 | 彻底移除 Mock driver。 |
...
| `Tauri.CommMapping/src-tauri/src/comm/error.rs` | 修改 | `CommRunErrorDetails` 新增 `missingFields?: [...]`（可选字段，保持兼容）。 |
| `Tauri.CommMapping/src-tauri/src/comm/usecase/run_validation.rs` | 新增 | run_start 前配置强校验（纯校验，不做 IO/通讯）。 |
| `Tauri.CommMapping/src-tauri/src/comm/tauri_api.rs` | 修改 | `comm_run_start_obs` 失败返回结构化 `ConfigError + missingFields`；driver 从 profiles 推断/校验协议匹配。 |
| `Tauri.CommMapping/src-tauri/src/comm/usecase/evidence_pack.rs` | 修改 | 测试数据里移除 `driver:\"mock\"` 字样（满足 0-hit grep）。 |
