...
- **验收证据**：
  ...
  - Run（start -> latest 出现 OK/Timeout/DecodeError 的日志片段；mock，不依赖真实 PLC）：
    ...
    ```
    - 生成方式：`cargo test --manifest-path Tauri.CommMapping/src-tauri/Cargo.toml run_engine_latest_contains_ok_timeout_and_decode_error_when_using_mock -- --nocapture`
...
- **验收自检**：
  ...
  - [x] `Tauri.CommMapping/Docs/ExecResults/TASK-12-result.md` 已归档（本文件）。
