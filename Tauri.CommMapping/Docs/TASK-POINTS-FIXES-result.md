...
### 后端（Rust / Tauri）
- `Tauri.CommMapping/src-tauri/src/comm/core/model.rs`
  - 新增 `CommProjectDataV1`（meta + connections/points/uiState 可选）与 `CommProjectUiStateV1`。
- `Tauri.CommMapping/src-tauri/src/comm/adapters/storage/projects.rs`
...
- `pnpm build`（关键片段）：
  ...
  - `✓ built in ...`
- `cargo test --manifest-path Tauri.CommMapping/src-tauri/Cargo.toml`（关键片段）：
  - `running 39 tests ... ok`
  - `running 2 tests ... ok`
