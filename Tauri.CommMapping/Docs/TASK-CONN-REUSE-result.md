...
- `Tauri.CommMapping/src-tauri/src/comm/adapters/driver/mod.rs`
...
- `Tauri.CommMapping/src-tauri/src/comm/adapters/driver/connection_manager.rs`
...
- `Tauri.CommMapping/src-tauri/src/comm/adapters/driver/modbus_tcp.rs`
  - `connect()`：只负责 `tcp::connect_slave(...)`
  - `read_with_client()`：复用同一个 `Context` 多次读
- `Tauri.CommMapping/src-tauri/src/comm/adapters/driver/modbus_rtu.rs`
  - `connect()`：只负责 `SerialStream::open + rtu::attach_slave(...)`
  - `read_with_client()`：复用同一个 `Context` 多次读

### 3) Engine 轮询结构：select(stop, tick) + do_poll_once() 复用连接
- `Tauri.CommMapping/src-tauri/src/comm/usecase/engine.rs`
...
1. 启动开发模式：在 `Tauri.CommMapping/src-tauri/` 目录运行 `cargo tauri dev`。
...
## Stop 为何能 < 1s 生效（说明）
- 轮询主循环：`Tauri.CommMapping/src-tauri/src/comm/usecase/engine.rs` 的 `CommRunEngine::start_run` 内部使用 `tokio::select!` 同时监听：
