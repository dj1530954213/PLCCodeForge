...
use crate::comm::tauri_api::{
    CommDriverKind, CommRunLatestResponse, CommRunStartRequest, CommState,
};
...
pub(crate) async fn start_run(
    ...
    let base_dir = comm_base_dir(&app, project_id.as_deref())
        .map_err(|message| error_from_message(message, None, project_id.clone(), device_id.clone()))?;
    ...
    let driver_kind = match request.driver {
        ...
        None => infer_driver_kind_from_profiles(&profiles.profiles)
            .map_err(|message| {
                error_from_message(message, None, project_id.clone(), device_id.clone())
            })?,
    };
...
pub(crate) fn error_from_message(
    ...
    CommRunError {
        kind: error_kind_from_message(&message),
        message,
        details: Some(CommRunErrorDetails {
...
fn error_kind_from_message(message: &str) -> CommRunErrorKind {
