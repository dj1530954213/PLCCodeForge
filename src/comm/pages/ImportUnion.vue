...
async function importAndGenerate() {
  ...
  try {
    ...
    const [existingPoints, existingProfiles] = await Promise.all([
      commPointsLoad(projectId.value).catch(() => ({ schemaVersion: 1, points: [] } as PointsV1)),
      commProfilesLoad(projectId.value).catch(() => ({ schemaVersion: 1, profiles: [] } as ProfilesV1)),
    ]);
