...
7) 验证方案（详细）
7.1 Master 仅新增验证
- 前置：选中 ETHERNET 节点；记录子节点列表（HTREEITEM + text）
- 动作：调用 OnAddProcotol("MODBUSTCP_MASTER")
- 成功判定（满足任意 2 条）：
  - Tree 子节点差分出现新增项，文本包含 MODBUSTCP_MASTER
  - MapNameToIdUpper("MODBUSTCP_MASTER") 命中 ID，MapIdToTree 可反查 HTREEITEM
  - MapTreeToId + GetDeviceByLogicID 返回有效对象（vtable 在 dllDPLogic）

7.2 Slave 仅新增验证（依赖 Master 已存在）
- 前置：选中 MODBUSTCP_MASTER 节点，ContextResolver 成功解析 commIdx/linkIdx/pLink/pParent
- 动作：调用 OnAddSlave(commIdx, linkIdx, "MODBUSSLAVE_TCP", desc/IP, count=1, extra=null)
- 成功判定（满足任意 2 条）：
  - Tree 子节点差分出现新增从站节点
  - NameMap/TreeMap 更新：MapNameToIdUpper + MapIdToTree 命中
  - 取设备对象后地址/IP 与 desc 一致（或符合自增规则）

7.3 串联验证（Master -> Slave）
- Master 新增后立即可被 ContextResolver 解析，Slave 节点能挂在 Master 下
- 失败时不继续执行 Slave；避免 UI/数据不一致

8) Master 节点定位策略（稳妥方案）
8.1 Tree 子节点差分（首选）
- 记录 ETHERNET 子节点列表 -> 调 OnAddProcotol -> 再抓列表 -> 差分取新增项
- 过滤文本：MODBUSTCP_MASTER / ModbusTCP_Master / HolliTCP_Master
- 验证：MapTreeToId + GetDeviceByLogicID

8.2 NameMap 快路径（备选）
- MapNameToIdUpper("MODBUSTCP_MASTER") -> MapIdToTree
- 需验证 Tree 文本与父节点为 ETHERNET

8.3 当前选中节点（兜底）
- TreeView_GetSelection 文本命中即可使用

8.4 ID 差分兜底
- 记录 MapIdToTree 的 ID 集合 -> 新增后差分 -> 取新增 ID -> MapIdToTree

9) 参数表与日志点（建议）
9.1 OnAddProcotol
- 参数：typeName="MODBUSTCP_MASTER"
- 前置：UI 线程；当前树选择在 ETHERNET
- 记录日志：返回值、Tree 差分结果、NameMap 命中情况

9.2 OnAddSlave
- 参数：commIdx/linkIdx/typeName="MODBUSSLAVE_TCP"/desc/IP/count=1/extra=null
- 前置：ContextResolver 解析出 link/parent
- 记录日志：返回值、新 ID、Tree 路径、NameMap 读写结果

9.3 失败判定
- OnAddProcotol/OnAddSlave 返回 0
- Tree 未出现新增节点且 NameMap 未更新
- ContextResolver 解析失败（Link/Parent 为空）

10) 更稳妥实现方式（细化）
10.1 线程模型
- 全部调用必须在 UI 线程（Timer/Afx 管理状态）触发，避免 TLS/MFC 崩溃。
- 原子化状态机建议：AddMaster -> LocateMaster -> ResolveCtx -> AddSlave -> Verify。

10.2 TreeScanner 主路径（定位 Master）
- 锁定 ETHERNET 节点句柄，抓取子节点快照（HTREEITEM + text）。
- 调用 OnAddProcotol 后再抓快照，用差分获取新增项。
- 过滤文本：MODBUSTCP_MASTER / ModbusTCP_Master / HolliTCP_Master。
- 验证父节点为 ETHERNET；优先差分结果作为 Master。

10.3 NameMap 快路径（辅助）
- MapNameToIdUpper("MODBUSTCP_MASTER") -> MapIdToTree。
- 若命中，必须二次验证 Tree 文本与父节点为 ETHERNET。

10.4 ContextResolver 要求
- 对 Master 节点执行 SafeResolve，要求 commIdx/linkIdx/pLink/pParent 均有效。
- 校验 vtable 在 dllDPLogic 范围内；失败则终止，不执行 OnAddSlave。

10.5 OnAddSlave 关键约束
- typeName 固定为 "MODBUSSLAVE_TCP"，count=1，extra=null。
- desc 使用 IP 字符串；必须在解析出的 linkIdx/commIdx 下执行。

10.6 异常处理策略
- OnAddProcotol 返回 0 直接终止（不做 Slave）。
- Tree/NameMap 均未更新则终止并记录。
- SafeResolve 失败则终止并提示“上下文异常”。

10.7 推荐日志点
- OnAddProcotol：返回值、Tree 差分结果、NameMap 命中情况。
- LocateMaster：候选节点数、最终选择节点文本/handle。
- SafeResolve：commIdx/linkIdx/pLink/pParent。
- OnAddSlave：返回值、新 ID、树路径、NameMap 读写结果。
