...
必须从 `src-tauri/` 目录运行（在 `src-tauri/src` 下会因找不到 tauri.conf 而失败）。
```bash
cd src-tauri
cargo tauri dev
```
...
cargo test --manifest-path src-tauri/Cargo.toml tcp_quality_ok_for_two_points_when_enabled -- --nocapture
```
...
## 风险与未决项
- RevoGrid 为 Web Component：若用户机器上 dev server 端口冲突，需同步修改 `vite.config.ts` 与 `src-tauri/tauri.conf.json` 的 `devUrl`。
