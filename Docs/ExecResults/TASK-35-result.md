...
- `Docs/Schemas/comm_ir.v1.schema.json`
  - CommIR v1 的最小可用 JSON Schema（字段/必填/枚举）。
- `Docs/Schemas/plc_import_bridge.v1.schema.json`
  - PlcImportBridge v1 的最小可用 JSON Schema（字段/必填/枚举；readArea 限定 Holding/Coil）。
- `Docs/Schemas/README.md`
  - 增加“演进策略”小节（v1 兼容承诺 + 何时 bump v2）。
- `src-tauri/src/comm/fixtures/comm_ir.sample.v1.json`
  - golden 输入：3 点位（Holding + Coil），结果包含 Ok + Timeout。
- `src-tauri/src/comm/fixtures/plc_import_bridge.expected.v1.json`
  - golden 期望输出（字段与数值固定）。
- `src-tauri/src/comm/bridge_plc_import.rs`
...
- [x] 新增 `Docs/Schemas/comm_ir.v1.schema.json`
- [x] 新增 `Docs/Schemas/plc_import_bridge.v1.schema.json`
