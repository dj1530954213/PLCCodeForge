...
## 改动清单
- `Autothink.UiaAgent/Flows/Autothink/AutothinkBuildFlow.cs`：BuildOutcome 判定 + StepLog 证据。
- `Autothink.UiaAgent.Stage2Runner/Program.cs`：summary.build 字段与 `build_outcome.json` 落盘。
- `Autothink.UiaAgent.DemoTarget/MainForm.cs`：statusLabel 同步 AccessibleName，确保 UIA 可读状态文本。
- `Docs/组态软件自动操作/RunnerConfig/demo.json`：buildOutcome 使用 `buildSucceededIndicator`（waitSelector）。
- `Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：buildOutcome 模板与签收说明。
...
来自 `logs/20260103-233612/summary.json`：
```json
{
  "build": {
    "outcome": "Success",
    "evidencePath": "C:\\Program Files\\Git\\code\\PLCCodeForge\\logs\\20260103-233612\\build_outcome.json"
  }
}
```
来自 `logs/20260103-233612/build_outcome.json`：
```json
{
  "outcome": "Success",
...
来自 `logs/20260103-234039/summary.json`：
```json
{
  "flows": [
    {
  ...
  "build": {
    "outcome": "Unknown",
    "evidencePath": "C:\\Program Files\\Git\\code\\PLCCodeForge\\logs\\20260103-234039\\build_outcome.json"
  }
}
```
来自 `logs/20260103-234039/build_outcome.json`：
```json
{
  "outcome": "Unknown",
