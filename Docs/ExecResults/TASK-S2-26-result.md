...
## 改动清单
- `Docs/组态软件自动操作/Selectors/autothink.v1.base.json`：v1 base 冻结 key 集合（DemoTarget 用 AutomationId）。
- `Docs/组态软件自动操作/Selectors/autothink.v1.local.sample.json`：真实 AUTOTHINK 模板（local 覆盖示例）。
- `Docs/组态软件自动操作/Selectors/README.md`：v1 pack 命名与覆盖约定。
- `Autothink.UiaAgent.Stage2Runner/Program.cs`：selector-check 门禁与 `selector_check_report.json` 落盘。
- `Docs/组态软件自动操作/RunnerConfig/demo.json`：启用 `selectorPackVersion: "v1"`。
- `Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：v1 pack 维护/校准说明。
...
来自 `logs/20260103-233612/selector_check_report.json`：
```json
{
  "packVersion": "v1",
  "missingKeys": [],
...
## 缺 key 失败证据（门禁生效）
来自 `logs/20260103-233808/selector_check_report.json`：
```json
{
  "missingKeys": [
    "autothink.build:(anyOf:buildSucceededIndicator|buildFinishedIndicator)"
...
```
来自 `logs/20260103-233808/summary.json`：
```json
{
  "flows": [
    {
