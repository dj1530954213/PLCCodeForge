...
- **完成摘要**：
  ...
  - Selector 资产落地到 `Docs/组态软件自动操作/Selectors/`，每个 flow 一份 JSON。
  - Runner 展开 selector，不改 Agent RPC 契约；兼容直接传 selector。

- **改动清单**：
  - `Autothink.UiaAgent.Stage2Runner/Program.cs`：新增 profile 加载、selector key 展开、CLI 参数。
  - `Docs/组态软件自动操作/Selectors/autothink.attach.json`：profile 模板。
  - `Docs/组态软件自动操作/Selectors/autothink.importProgram.textPaste.json`：profile 模板（含 editor/buildButton 示例）。
  - `Docs/组态软件自动操作/Selectors/autothink.importVariables.json`：profile 模板。
  - `Docs/组态软件自动操作/Selectors/autothink.build.json`：profile 模板。
  - `Docs/组态软件自动操作/Selectors/README.md`：命名与 schema 说明。
...
- **Stage2Runner 运行日志片段（示例）**：
  ```text
  SelectorsRoot: C:\...\Docs\组态软件自动操作\Selectors
  Profile: autothink
