...
## 改动清单
- `Autothink.UiaAgent.Stage2Runner/Program.cs`：新增 `stoppedBecause`、fail-fast 逻辑、`allowPartial` 配置支持；缺失 selectorKey 时输出结构化 summary 而非崩溃。
- `Docs/组态软件自动操作/RunnerConfig/demo.json`：默认不再 skip build，形成完整流水线。
- `Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：新增 fail-fast/skip/allowPartial 说明。
...
```
日志目录：`C:\Program Files\Git\code\PLCCodeForge\logs\20260103-211408`
...
```
日志目录：`C:\Program Files\Git\code\PLCCodeForge\logs\20260103-211925`
