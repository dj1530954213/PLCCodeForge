...
- **改动清单**：
  - `Autothink.UiaAgent/AgentHost.cs`：RPC target 切换为 `UiaRpcService`。
  - `Autothink.UiaAgent.Stage2Runner/Program.cs`：RunnerConfig 增加 `agentPath` 支持。
  - `Docs/组态软件自动操作/RunnerConfig/demo.json`：补充 `agentPath` 指向 FullHost。
  - `Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：新增 FullHost 启动说明与 --check 步骤。
...
- **--check 成功示例**：
  - stdout 片段：
    ```text
    RunDir: C:\Program Files\Git\code\PLCCodeForge\logs\20260103-181038
    Connectivity: C:\Program Files\Git\code\PLCCodeForge\logs\20260103-181038\connectivity.json
    Connectivity check: OK
    ```
  - `logs/20260103-181038/connectivity.json` 片段：
    ```json
    {
      "ok": true,
      "handshakeReady": true,
...
- **Runbook 更新片段**：
  ...
  - 也可手动启动：dotnet run --project Autothink.UiaAgent/Autothink.UiaAgent.csproj -c Release
  ```
