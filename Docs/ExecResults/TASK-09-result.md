...
- **完成摘要**：
  - 在 `src-tauri/src/comm/tauri_api.rs` 定义并暴露通讯采集模块的 Tauri commands（profiles/points/plan/run/export），并将对外 DTO 视为冻结契约（只允许新增可选字段）。
  - 在 `src-tauri/src/lib.rs` 通过 `tauri::Builder::manage(CommState::new())` 注入全局状态，并注册 commands 到 `invoke_handler`。
  - start/stop/latest 语义满足“command 不阻塞 UI”硬约束（已写入 `src-tauri/src/comm/tauri_api.rs` 顶部注释）：
...
- **完成证据**：
  - `cargo build --manifest-path src-tauri/Cargo.toml` 输出片段：
    ...
    ```
  - `src-tauri/src/lib.rs` 注册 commands 片段：
    ```rust
...
- **验收自检**：
  - [x] commands 已在 `src-tauri/src/comm/tauri_api.rs` 定义并在 `src-tauri/src/lib.rs` 注册。
  ...
  - [x] DTO 契约冻结说明已写入 `src-tauri/src/comm/tauri_api.rs` 顶部注释。
  - [x] `Docs/ExecResults/TASK-09-result.md` 已归档（本文件）。
