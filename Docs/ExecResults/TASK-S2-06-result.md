...
- **改动清单**：
  - `Autothink.UiaAgent/Flows/Autothink/AutothinkImportVariablesFlow.cs`：导入变量表完整流程实现。
  - `Autothink.UiaAgent/Flows/FlowRegistry.cs`：注册真实 flow。
  - `Autothink.UiaAgent.Tests/AutothinkImportVariablesArgsTests.cs`：新增参数校验单测。
