...
## 改动清单
- `Autothink.UiaAgent/Flows/Autothink/AutothinkImportProgramTextPasteFlow.cs`：新增 clipboard 失败时的 `SendKeysPaste` Warning 记录，保证证据链完整；保留 fallback 行为。
- `Autothink.UiaAgent.DemoTarget/MainForm.cs`：修复变量导入对话框非模态展示的生命周期，确保 DemoTarget 可稳定联调。
- `Docs/组态软件自动操作/Selectors/autothink.base.json`：补齐 DemoTarget 对应的 importProgram 关键 selector key。
- `Docs/组态软件自动操作/Selectors/autothink.importProgram.local.sample.json`：真实 AUTOTHINK 模板。
- `Docs/Samples/program_demo.st`：DemoTarget 用 ST 文本样例。
- `Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：新增 importProgram 校准说明。
...
```json
{
  ...
  "ok": true,
  "root": "mainWindow",
  "logFile": "C:\\Program Files\\Git\\code\\PLCCodeForge\\logs\\20260103-211408\\autothink.importProgram.textPaste.json"
}
```
...
## Selector 资产片段
`Docs/组态软件自动操作/Selectors/autothink.base.json`：
```json
{
...
- 真实 AUTOTHINK 模板：Docs/组态软件自动操作/Selectors/autothink.importProgram.local.sample.json
```
