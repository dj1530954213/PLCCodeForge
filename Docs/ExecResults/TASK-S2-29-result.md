...
## 改动清单
- `Autothink.UiaAgent.Stage2Runner/Program.cs`：UIStateRecovery 管线、`unexpected_ui_state.json` 落盘、summary.uiStateRecovery 字段。
- `Docs/组态软件自动操作/Selectors/autothink.v1.base.json`：新增 `global.*` keys。
- `Docs/组态软件自动操作/Selectors/autothink.v1.local.sample.json`：新增 `global.*` keys 模板。
- `Autothink.UiaAgent.DemoTarget/MainForm.cs`：启动提示弹窗（OK）。
- `Docs/组态软件自动操作/Runbook-Autothink-普通型.md`：新增 UIStateRecovery 使用说明。
...
2. 运行 Stage2Runner：`Autothink.UiaAgent.Stage2Runner.exe --config Docs/组态软件自动操作/RunnerConfig/demo.json`
3. 启动提示弹窗自动被关闭，流程继续执行。

## summary.json 片段（uiStateRecovery）
来自 `logs/20260104-004044/summary.json`：
```json
{
  "uiStateRecovery": {
    "attempts": 3,
    ...
    "evidencePath": "C:\\Program Files\\Git\\code\\PLCCodeForge\\logs\\20260104-004044\\unexpected_ui_state.json"
  }
}
```

## unexpected_ui_state.json 片段
来自 `logs/20260104-004044/unexpected_ui_state.json`：
```json
{
  "attempts": 3,
  "handled": 1,
...
来自 `logs/20260104-004044/step_logs.json`：
```json
{
...
来自 `Docs/组态软件自动操作/Selectors/autothink.v1.base.json`：
```json
"global.popupRoot": {
  "Path": [
