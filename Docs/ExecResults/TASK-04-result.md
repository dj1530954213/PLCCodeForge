...
- **完成摘要**：
  - 在 `src-tauri/src/comm/plan.rs` 实现 `ReadPlan` 构建：按 `channelName` 分组、按 points 顺序做地址自动映射、对连续地址聚合并按最大长度分批。
...
- **改动清单**：
  ...
  - `Docs/ExecResults/TASK-04-result.md`
    - 新建任务结果归档文件（本文件）
...
- **验收自检**：
  ...
  - [x] `Docs/ExecResults/TASK-04-result.md` 已归档。
