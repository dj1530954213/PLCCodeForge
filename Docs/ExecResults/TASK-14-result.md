...
- **完成摘要**：
  - 新增集成测试 `src-tauri/tests/comm_it.rs`：
  ...
  - 为了让 integration test 可引用 comm 模块：`src-tauri/src/lib.rs` 将 `comm` 提升为 `pub mod comm;`（不影响 Tauri command 契约）。

- **改动清单**：
  - `src-tauri/src/lib.rs`
    - 调整：`pub mod comm;`（供 `src-tauri/tests/*.rs` 引用）
  - `src-tauri/tests/comm_it.rs`
    - 调整：ENV 列表对齐冻结口径（仅 `COMM_IT_ENABLE` + TCP/RTU 固定字段）
    - 新增：无 ENV 时的 `SKIP` 输出；打印 `RawReadData` 作为人工核对证据
  - `Docs/ExecResults/TASK-14-result.md`
...
- **如何启用（运行命令/环境变量）**：
  ...
  - **运行命令**：
    - `cargo test --manifest-path src-tauri/Cargo.toml --test comm_it -- --nocapture`
...
- **验收自检**：
  ...
  - [x] `Docs/ExecResults/TASK-14-result.md` 已归档
