...
use crate::comm::core::model::{
    CommProjectDataV1, CommProjectV1, PointsV1, ProfilesV1, SCHEMA_VERSION_V1,
};
...
pub fn create_project(
    ...
    // Pre-create empty profiles/points to make "open project" deterministic.
    let empty_profiles = ProfilesV1 {
        schema_version: SCHEMA_VERSION_V1,
        profiles: Vec::new(),
    };
    storage::save_profiles(&comm_dir, &empty_profiles)
    ...
    let project_data = CommProjectDataV1 {
        ...
        connections: Some(empty_profiles),
...
pub fn load_project_data(
    ...
    let connections = if value.get("connections").is_some() {
...
pub fn save_project_data(app_data_dir: &Path, payload: &CommProjectDataV1) -> Result<(), String> {
    ...
    let Some(connections) = payload.connections.as_ref() else {
    ...
    let Some(points) = payload.points.as_ref() else {
    ...
    ensure_project_dirs(&comm_dir).map_err(|e| e.to_string())?;
    write_json_atomic(&project_meta_path(&comm_dir), payload).map_err(|e| e.to_string())?;
...
pub fn copy_project(
    ...
    // Ensure `project.v1.json` includes copied connections/points (single-source-of-truth).
    let connections = storage::load_profiles(&dst_comm_dir)
        .map_err(|e| e.to_string())?
        .unwrap_or(ProfilesV1 {
            schema_version: SCHEMA_VERSION_V1,
            profiles: Vec::new(),
        });
    let points = storage::load_points(&dst_comm_dir)
        .map_err(|e| e.to_string())?
        .unwrap_or(PointsV1 {
            schema_version: SCHEMA_VERSION_V1,
            points: Vec::new(),
